...
func (s *CommentRepositoryTestSuite) SetupSuite() { s.mt = mtest.New(s.T(), mtest.NewOptions().ClientType(mtest.Mock)) }

func (s *CommentRepositoryTestSuite) SetupTest() { s.mt.Run("test", func(mt *mtest.T) { s.repo = repositories.NewCommentRepository(mt.Coll) }) }
...
func (s *CommentRepositoryTestSuite) TestCreateComment_Success() {
    s.mt.Run("test", func(mt *mtest.T) {
        ...
        s.NoError(err); s.NotNil(res); s.Equal("hi", res.Content); s.NotZero(res.ID)
    })
}
...
func (s *CommentRepositoryTestSuite) TestGetCommentsByPost_Success() {
    s.mt.Run("test", func(mt *mtest.T) {
        s.repo = repositories.NewCommentRepository(mt.Coll)
        postID := primitive.NewObjectID(); authorID := primitive.NewObjectID()
        ...
        s.NoError(err); s.Equal(int64(1), total); s.Len(list, 1); s.Equal("hello", list[0].Content)
    })
}
...
func (s *CommentRepositoryTestSuite) TestUpdateComment_Success() {
    s.mt.Run("test", func(mt *mtest.T) {
        s.repo = repositories.NewCommentRepository(mt.Coll)
        id := primitive.NewObjectID(); postID := primitive.NewObjectID(); authorID := primitive.NewObjectID()
        ...
        s.NoError(err); s.NotNil(res); s.Equal("updated", res.Content)
    })
}
