...
func (s *CommentControllerTestSuite) SetupTest() {
    ...
    s.router.Use(func(c *gin.Context) {
        if c.GetHeader("Authorization") != "" { c.Set("userID", "507f1f77bcf86cd799439011") }
        c.Next()
    })
...
func (s *CommentControllerTestSuite) performRequest(method, url string, body interface{}, headers map[string]string) *httptest.ResponseRecorder {
    ...
    if body != nil { b,_ := json.Marshal(body); reqBody = bytes.NewBuffer(b) } else { reqBody = bytes.NewBuffer(nil) }
    req, _ := http.NewRequest(method, url, reqBody)
    req.Header.Set("Content-Type", "application/json")
    for k,v := range headers { req.Header.Set(k,v) }
    w := httptest.NewRecorder(); s.router.ServeHTTP(w, req); return w
}

func (s *CommentControllerTestSuite) TestCreateComment_Success() {
    postID := primitive.NewObjectID(); uid,_ := primitive.ObjectIDFromHex("507f1f77bcf86cd799439011")
...
func (s *CommentControllerTestSuite) TestCreateComment_Unauthorized() {
    postID := primitive.NewObjectID(); req := commentpkg.CreateCommentRequest{Content: "Hi"}
...
func (s *CommentControllerTestSuite) TestDeleteComment_Success() {
    cid := primitive.NewObjectID(); uid,_ := primitive.ObjectIDFromHex("507f1f77bcf86cd799439011")
