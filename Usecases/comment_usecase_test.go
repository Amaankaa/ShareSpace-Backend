...
func (s *CommentUsecaseTestSuite) TearDownTest() { s.commentRepo.AssertExpectations(s.T()); s.postRepo.AssertExpectations(s.T()); s.userRepo.AssertExpectations(s.T()) }

func (s *CommentUsecaseTestSuite) TestCreateComment_Success() {
    postID := primitive.NewObjectID(); uid := primitive.NewObjectID()
    ...
    s.NoError(err); s.NotNil(resp); s.Equal("hi", resp.Content)
}

func (s *CommentUsecaseTestSuite) TestUpdateComment_Unauthorized() {
    cid := primitive.NewObjectID(); uid := primitive.NewObjectID()
    ...
    s.Error(err); s.Contains(err.Error(), "unauthorized")
}

func (s *CommentUsecaseTestSuite) TestDeleteComment_Success() {
    cid := primitive.NewObjectID(); uid := primitive.NewObjectID(); postID := primitive.NewObjectID()
